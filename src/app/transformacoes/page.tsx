    ...
    <div className="text-white min-h-screen">
      ...
      <main className="relative z-0">
        ...
        <section className="px-4 md:px-6 lg:px-8 pb-10">
          <div className="max-w-7xl mx-auto columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">{transformations.slice(0, 6).map((trans) => (<TransformationCard key={trans.id} trans={trans} />))}</div>
        </section>

        <PersuasiveSection title="Cada Foto, Uma Nova História de Sucesso" glowPosition="left"><p>O que você vê não são apenas mudanças estéticas, mas uma prova de que a dedicação, guiada pelo método certo, gera resultados reais e duradouros. A próxima história pode ser a sua.</p></PersuasiveSection>

        <section className="px-4 md:px-6 lg:px-8 py-10">
          <div className="max-w-7xl mx-auto columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">{transformations.slice(6, 12).map((trans) => (<TransformationCard key={trans.id} trans={trans} />))}</div>
        </section>

        <section className="py-20 bg-black/30 backdrop-blur-sm">
            <div className="max-w-7xl mx-auto px-4">
                <div className="text-center mb-12"><h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4">A Opinião Detalhada de Quem Vive a Experiência</h2><p className="text-gray-400 max-w-2xl mx-auto text-sm sm:text-base">Palavras que recebemos e que mostram o verdadeiro impacto do nosso trabalho.</p></div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">{testimonials.map((testimonial, index) => (<TestimonialCard key={index} testimonial={testimonial} />))}</div>
            </div>
        </section>

        <section className="relative py-16 sm:py-20 md:py-28 px-4">
          <div className="absolute inset-0 -z-10 flex items-center justify-center opacity-50"><div className="w-[30rem] sm:w-[40rem] md:w-[50rem] h-[30rem] sm:h-[40rem] md:h-[50rem] bg-green-500/15 rounded-full blur-3xl" /></div>
          <div className="max-w-4xl mx-auto text-center">
            <div className="bg-gray-900/70 backdrop-blur-md p-6 sm:p-8 md:p-16 rounded-2xl border border-gray-800 shadow-2xl shadow-green-500/10">
              ...
              <div className="mt-8 sm:mt-10 flex flex-col sm:flex-row items-center justify-center gap-4 max-w-md sm:max-w-none mx-auto">
                  <Link href="/consultoria" className="w-full sm:w-auto"><button className="w-full sm:w-auto min-w-[200px] bg-green-500 text-black font-bold py-3 px-6 sm:px-8 rounded-lg hover:bg-green-600 transition-all text-sm sm:text-base">Quero Consultoria Online</button></Link>
                  <Link href="/treinopersonalizado" className="w-full sm:w-auto"><button className="w-full sm:w-auto min-w-[200px] bg-transparent border-2 border-amber-400 text-amber-400 font-bold py-3 px-6 sm:px-8 rounded-lg hover:bg-amber-400 hover:text-black transition-all text-sm sm:text-base">Quero a Planilha de Treino</button></Link>
              </div>
            </div>
          </div>
        </section>
